...
def _validate_python_function_args(
    code_file: Path,
) -> list[dict]:
    """
    ...
    Uses ast.parse() to extract FunctionDef nodes and their parameter counts,
    then walks Call nodes to check that argument counts fall within the valid range
    (accounting for defaults, *args, and **kwargs).

    Only validates calls to functions defined in the same file. Method calls
    (attribute access like obj.method()) and calls to external/imported functions
    are skipped.

    Returns:
        List of dicts describing mismatches. Each dict has keys:
        - function: the function name
        - expected_min: minimum required positional args
        - expected_max: maximum positional args (None if *args)
        - actual: number of positional args in the call
    ...
    # Collect function definitions with their parameter info.
    # Skip methods inside classes (they have implicit self/cls).
    ...
    # Walk all Call nodes and check argument counts
    mismatches: list[dict] = []
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue

        # Only check simple name calls (not attribute access like obj.method())
                        ...
                        try:
                            if operation == 'auto-deps':
                                ...
                                if agentic_mode and language.lower() == 'python' and pdd_files['code'].exists():
                                    ...
                                    # Issue #625: Validate function call argument counts after generation
                                    arg_mismatches = _validate_python_function_args(
                                        pdd_files['code'],
                                    )
                                    if arg_mismatches:
                                        def _fmt_expected(m):
                                        ...
                                        error_msg = (
                                            f"Function argument mismatch in generated code: "
                                            f"{mismatch_details}. "
                                            f"Function calls do not match their definitions in the same file."
                                        )
                                        errors.append(error_msg)
                                        log_event(basename, language, "function_arg_validation_failed", {
