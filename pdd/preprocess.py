...
# Module-level quiet flag, set by preprocess(quiet=True)
_quiet_mode = False
...
def preprocess(prompt: str, recursive: bool = False, double_curly_brackets: bool = True, exclude_keys: Optional[List[str]] = None, quiet: bool = False) -> str:
    effective_quiet = _quiet_mode or quiet
    try:
        if not prompt:
        ...
        if double_curly_brackets:
            prompt = double_curly(prompt, exclude_keys, quiet=effective_quiet)
