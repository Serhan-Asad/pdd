...
![PDD-CLI Version](https://img.shields.io/badge/pdd--cli-v0.0.103-blue) [![Discord](https://img.shields.io/badge/Discord-join%20chat-7289DA.svg?logo=discord&logoColor=white)](https://discord.gg/Yp4RTh8bG7)
...
- `ERROR_FILE`: (**Required in non-loop mode only**) The filename containing the unit test runtime error messages. **Must NOT be provided when using `--loop` mode** (errors are auto-generated by the verification program).
...
```bash
pdd [GLOBAL OPTIONS] fix --loop --verification-program PATH [OTHER OPTIONS] PROMPT_FILE CODE_FILE UNIT_TEST_FILE...
```

Or you may want to enable it explicitly

```bash
pdd [GLOBAL OPTIONS] fix --loop --verification-program PATH --agentic-fallback [OTHER OPTIONS] PROMPT_FILE CODE_FILE UNIT_TEST_FILE...
```

To disable this feature while using `--loop`, add `--no-agentic-fallback` to turn it off.

```bash
pdd [GLOBAL OPTIONS] fix --loop --verification-program PATH --no-agentic-fallback [OTHER OPTIONS] PROMPT_FILE CODE_FILE UNIT_TEST_FILE...
```
