...
5. **Root cause analysis** - Run experiments to identify the root cause. Classifies bug complexity as `simple`, `medium`, or `complex` to scale downstream test generation. Posts comment explaining the root cause and complexity classification.
...
6. **Test plan** - Design a plan for creating tests to detect the problem. Scales test depth based on complexity: simple bugs get 1 test, medium get 2-4, complex get the full suite. Posts comment with the test plan.
...
9. **E2E test** - Generate and run end-to-end tests to verify the bug at integration level. Skipped for simple bugs (unit test sufficient), lightweight for medium bugs, full suite for complex bugs. Posts comment with E2E test results or skip reason.
